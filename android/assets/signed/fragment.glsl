...
uniform vec2 u_mousePos;        //mouse pixel coords. xy
...
float sceneDist(vec2 p)
{
	...
	float t2 = triangleDist(	rotateCW(translate(p, vec2(600, 250)), u_time), 40.0);
	...
	float b3 = boxDist(		translate(p, vec2(100, sin(u_time * 3.0 + 1.0) * 40.0 + 100.0)),
					   		vec2(40, 15), 	0.0);
	float c2 = circleDist(	translate(p, vec2(100, 100)),	30.0);
	float s = substract(b3, c2);

	float b4 = boxDist(		translate(p, vec2(200, sin(u_time * 3.0 + 2.0) * 40.0 + 100.0)),
					   		vec2(40, 15), 	0.0);
	float c3 = circleDist(	translate(p, vec2(200, 100)), 	30.0);
	float i = intersect(b4, c3);

	float b5 = boxDist(		translate(p, vec2(300, sin(u_time * 3.0 + 3.0) * 40.0 + 100.0)),
					   		vec2(40, 15), 	0.0);
...
void main()
{
	...
	// gradient
	vec4 col = vec4(0.5, 0.5, 0.5, 1.0)* (1.0 - length(c - p)/u_resolution.x);
	// grid
